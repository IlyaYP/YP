...
import (
	...
	"compress/flate"
	"fmt"
	"github.com/andybalholm/brotli" // go get -u github.com/andybalholm/brotli
	"log"
	"strings"
)
...
func main() {
	...
	// сжимаем содержимое data c brotli
	// go get -u github.com/andybalholm/brotli
	b1, err := BrotliCompress(data)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("%d bytes has been compressed to %d bytes with BrotliCompress\r\n", len(data), len(b1))
...
func BrotliCompress(data []byte) ([]byte, error) {
	...
	w := brotli.NewWriterLevel(&buf, brotli.BestCompression) // go get -u github.com/andybalholm/brotli
	_, err := w.Write(data)
	if err != nil {
		return nil, err
	}
